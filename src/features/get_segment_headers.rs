...
use subspace_networking::{Node, SegmentHeaderResponse, SegmentHeaderRequest};
use tracing::{info, warn};

pub async fn get_segment_headers(dsn_node: Node, peer_id: PeerId, last: bool) -> Vec<SegmentHeader> {
    let request = if last{
        SegmentHeaderRequest::LastSegmentHeaders {
            segment_header_number: 10,
        }
    } else{
        SegmentHeaderRequest::SegmentIndexes {segment_indexes: vec![4.into()]}
    };
